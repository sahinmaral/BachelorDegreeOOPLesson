...
public class CandidateForeignLanguageManager implements CandidateForeignLanguageService {
    ...
    public Result deleteById(int id) {
        var foreignLanguage = candidateForeignLanguageDao.getCandidateForeignLanguageById(id);

        if (foreignLanguage==null)
            return new ErrorResult(Messages.CandidateForeignLanguageDoesntExist);

        candidateForeignLanguageDao.delete(foreignLanguage);
    ...
    public Result deleteByCvId(int cvId) {
        ...
        var foreignLanguages = candidateForeignLanguageDao.getCandidateForeignLanguagesByCv_Id(cvId);

        if (foreignLanguages.size() == 0)
            return new ErrorResult(Messages.NoCandidateForeignLanguageOnCv);
        else
        {
            for (var foreignLanguage : foreignLanguages)
                candidateForeignLanguageDao.delete(foreignLanguage);
            return new SuccessResult(Messages.SuccessfullyDeleted);
